...
public class OmsPromotionServiceImpl implements OmsPromotionService {
    ...
    private Map<Long, List<OmsCartItem>> groupCartItemBySpu(List<OmsCartItem> cartItemList) {
        ...
        for(OmsCartItem item:cartItemList){
            List<OmsCartItem> omsCartItems = map.get(item.getProductId());
            if(omsCartItems==null){
                ArrayList<OmsCartItem> omsCartItems1 = new ArrayList<>();
                omsCartItems1.add(item);
                map.put(item.getProductId(),omsCartItems1);
            }
            else{
                omsCartItems.add(item);
            }
